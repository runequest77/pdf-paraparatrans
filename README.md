...
→「使い方」セクションへ！
    ...
    > exeでの利用では、Windowsやセキュリティソフトが`Trojan:Win32/***`としてダウンロードしたexeを削除してしまうことが多いです。
    > 使う方が楽かと思ってexeを作ってみましたが、解除するのも結構大変なので、**ローカルセットアップがオススメ** です。
    >
    > Chat-GPTに解除方法の一例を答えてもらいました。セキュリティソフトによっても違うので、自己責任で解除して下さい。
